...
Unit t_TemperatureControl_Fan_i_fp_fan_seL4Nix_fanAck_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Fan_i.c", "", "t_TemperatureControl_Fan_i_fp_fan_seL4Nix_fanAck_Send", 0);

  sb_fanAck_enqueue(d);
}

// is_empty fanCmd: In EventDataPort
B t_TemperatureControl_Fan_i_fp_fan_seL4Nix_fanCmd_IsEmpty(STACK_FRAME_ONLY) {
  return sb_fanCmd_is_empty();
}

// receive fanCmd: In EventDataPort union_art_DataContent
Unit t_TemperatureControl_Fan_i_fp_fan_seL4Nix_fanCmd_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Fan_i.c", "", "t_TemperatureControl_Fan_i_fp_fan_seL4Nix_fanCmd_Receive", 0);
...
void pre_init(void) {
  ...
  printf("Entering pre-init of Fan_i_fp_fan\n");
  ...
  t_TemperatureControl_Fan_i_fp_fan_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  t_TemperatureControl_Fan_i_fp_fan_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of Fan_i_fp_fan\n");
}
...
int run(void) {
  ...
  for(;;) {
    ...
    t_TemperatureControl_Fan_i_fp_fan_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
