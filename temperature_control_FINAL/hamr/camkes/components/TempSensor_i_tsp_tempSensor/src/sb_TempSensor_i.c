...
Unit t_TemperatureControl_TempSensor_i_tsp_tempSensor_seL4Nix_currentTemp_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_TempSensor_i.c", "", "t_TemperatureControl_TempSensor_i_tsp_tempSensor_seL4Nix_currentTemp_Send", 0);

  sb_currentTemp_write(d);
}

// send tempChanged: Out EventPort
Unit t_TemperatureControl_TempSensor_i_tsp_tempSensor_seL4Nix_tempChanged_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_TempSensor_i.c", "", "t_TemperatureControl_TempSensor_i_tsp_tempSensor_seL4Nix_tempChanged_Send", 0);
...
void pre_init(void) {
  ...
  *sb_tempChanged_counter = 0;
  ...
  t_TemperatureControl_TempSensor_i_tsp_tempSensor_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  t_TemperatureControl_TempSensor_i_tsp_tempSensor_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    t_TemperatureControl_TempSensor_i_tsp_tempSensor_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
