...
abstract class TempSensor_i_tsp_tempSensor_TestApi extends BridgeTestSuite[TempSensor_i_tsp_tempSensor_Bridge](Arch.TempControlSystem_i_Instance_tsp_tempSensor) {

  /** helper function to check TempSensor_i_tsp_tempSensor's
   ...
   */
  def check_concrete_output(currentTemp: TemperatureControl.Temperature_i => B = currentTempParam => {T},
                            tempChanged: Z => B = tempChangedParam => {T}): Unit = {
    var testFailures: ISZ[ST] = ISZ()

    val currentTempValue: TemperatureControl.Temperature_i = get_currentTemp().get
