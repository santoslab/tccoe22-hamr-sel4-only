...
import art._
import t._
import t.TemperatureControl.{TempControl_i_tcp_tempControl => component}
    ...
    def compute(): Unit = {
      ...
      for(portId <- dispatchableEventPorts) {
        if(portId == fanAck_Id){
          ...
          // implement the following in 'component':  def handle_fanAck(api: TempControl_i_Operational_Api, value: TemperatureControl.FanAck.Type): Unit = {}
          ...
          // implement the following in 'component':  def handle_setPoint(api: TempControl_i_Operational_Api, value: TemperatureControl.SetPoint_i): Unit = {}
