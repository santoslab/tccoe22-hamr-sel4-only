...
Unit t_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B t_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit t_Process_sleep(STACK_FRAME Z n) {
